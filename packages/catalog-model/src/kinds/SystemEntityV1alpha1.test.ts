...
describe('SystemV1alpha1Validator', () => {
  ...
  it('rejects empty contains', async () => {
    (entity as any).spec.contains = [''];
    await expect(validator.check(entity)).rejects.toThrow(/contains/);
  });

  it('rejects undefined contains', async () => {
    (entity as any).spec.contains = [undefined];
    await expect(validator.check(entity)).rejects.toThrow(/contains/);
  });

  it('accepts no contains', async () => {
    (entity as any).spec.contains = [];
    await expect(validator.check(entity)).resolves.toBe(true);
  });
  ...
  it('rejects wrong partOf', async () => {
    (entity as any).spec.partOf = 7;
